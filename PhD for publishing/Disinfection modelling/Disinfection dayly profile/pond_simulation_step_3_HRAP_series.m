...
if d_i < 0.8*d_obj %if the depth falls below a limit, a volume of wastewater is added for compensation
    V_add = (d_obj - d_i) * S; % leave the possibility here to take a volume added as an ouput and compute the water footprint of the algal pond. Note the addition criteria can be changed, could be used easily in the input.
else
    V_add = 0;
end
...
if d_i > 2*d_obj % correction if depth is getting to high
...
diff = (qi + qr_i - qe)*s_t - V_out; % Volume variation
DF = (d_i + (qr_i - qe)*s_t/S)/d_i; % Dilution factor due to rain and evaporation;
