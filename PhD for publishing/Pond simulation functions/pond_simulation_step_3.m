...
% bCOD: biodegradable Chemical oxygen demand (g VSS/m3)
% X_bacteria: heterotroph concentration (g VSS/m3)
% C_coli: E. coli concentration (MPN/m3)
% X_debris_a: debris fraction due to decay of algae (kg VSS/m3)
% X_debris_b: debris fraction due to decay of bacteria  (g VSS/m3)
% X_nit: Nitrifier concentration (g VSS/m3)
% DO: dissolved oxygen concentration (mg/L)
% pH
% H2CO3: carbonic acid concentration (mg-C/L)
% HCO3: bicarbonate concentration (mg-C/L)
% CO3: carbonate concentration (mg-C/L)
% IC: Total inorganic carbon (mol/L)
% H: H+ concnetration (mol/L)
% OH: OH- concentration (mg/L)
% NH3: NH3 concentration (mg N/L)
% NH4: NH4+ concentration (mg N/L)
% IN: total amoniacal N (mg N/L)
% H2PO4: H2PO4- concentration (mg P/L)
% HPO4: HPO42- concentration (mg P/L)
% PO4: PO43- concentration(mg P/L)
% IP: total inorganic phosphorous (mg P/L)
% Sigam: Pond inert charge balance (meq/L)
...
% The depth is now set to be the min between d_obj and natural depth due to
% imbalance between inlet and outlet; The outlet flow rate is equal to all
% the flow due to water above d_obj.
    if diff > 0                                                                                    
        if d_i < d_obj
    ...
    % In case depth is too low (< 0.8*depth design), a volume of wastewater
    % is added.
    if d < 0.8*d_obj
        V_add = (d_obj - d) * S; % leave the possibility here to take a volume added as an ouput and compute the water footprint of the algal pond.
        d = d_obj;
    end
...
% Calculations of inorganic IC, IP, and IN
